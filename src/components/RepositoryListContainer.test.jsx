...
import { formatNumber } from "./RepositoryInfoBox";
...
describe("RepositoryList", () => {
  describe("RepositoryListContainer", () => {
    it("renders repository information correctly", () => {
      ...
      const repositoryItems = repositories.edges.map((n) => n.node);

      repositoryItems.forEach(function (item, index) {
        ...
        expect(getAllByTestId("repositoryStars")[index]).toHaveTextContent(
          formatNumber(item.stargazersCount)
        );

        expect(
          getAllByTestId("repositoryRatingAverage")[index]
        ).toHaveTextContent(formatNumber(item.ratingAverage));

        expect(
          getAllByTestId("repositoryReviewCount")[index]
        ).toHaveTextContent(formatNumber(item.reviewCount));
      });
    });
  });
});
